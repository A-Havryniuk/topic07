...
public class DBManager {
	...
	public User getUser(String login) throws DBException {
		try (Connection connection = DriverManager.getConnection(getFullURL());
		Statement statement = connection.createStatement()) {
		ResultSet rs = statement.executeQuery(Constants.SELECT_USER.replaceAll("[?]", "'"+login+"'"));
		return new User(rs.getInt(Fields.USER_ID), login);
		} catch (SQLException ex)
		{
			ex.printStackTrace();
		}
	...
	public Team getTeam(String name) throws DBException {
		try (Connection connection = DriverManager.getConnection(getFullURL());
			 Statement statement = connection.createStatement()) {
			ResultSet rs = statement.executeQuery(Constants.SELECT_TEAM.replaceAll("[?]", "'"+name+"'"));
			return new Team(rs.getInt(Fields.TEAM_ID), name);
		} catch (SQLException ex)
		{
			ex.printStackTrace();
		}
	...
	public List<Team> getUserTeams(User user) throws DBException {
		List<Team> user_team= new ArrayList<>();
		try (Connection connection = DriverManager.getConnection(getFullURL());
			 Statement statement = connection.createStatement()) {
			ResultSet rs = statement.executeQuery(Constants.GET_USERS_TEAMS);
			while(rs.next()) {
				teams.add(mapTeamByTeamId(rs));
			}
			rs.close();
		} catch (SQLException ex)
		{
	...
	public boolean updateTeam(Team team) throws DBException {
		try(Connection connection = DriverManager.getConnection(getFullURL());
		Statement statement = connection.createStatement()) {
			statement.executeUpdate(Constants.UPDATE_TEAM.replaceAll("[?]", team.getId()+"").replaceAll("[!]", team.getName()));
			return true;
		} catch (SQLException ex) {
			ex.printStackTrace();
		}
